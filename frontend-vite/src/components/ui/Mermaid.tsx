...
const Mermaid: React.FC<MermaidProps> = ({ chart, id = 'mermaid-diagram', className = '' }) => {
    ...
    useEffect(() => {
        ...
        const sanitizeMermaid = (content: string): string => {
            ...
            const isQ = (s: string) => s.trim().startsWith('"');

            return lines.map((line, idx) => {
                if (idx === 0) return line;
                const trimmed = line.trim();
                if (!trimmed || trimmed.startsWith('%%')) return line;
                if (/^\s*(style|classDef|class|linkStyle)\s/.test(line)) return line;
                if (/^\s*end\s*$/.test(trimmed)) return line;

                // subgraph — quote title if unquoted
                if (/^\s*subgraph\s+/i.test(line)) {
                    return line.replace(/^(\s*subgraph\s+)(.+)$/, (_m, p, t) =>
                        t.trim().startsWith('"') ? line : `${p}${q(t)}`
                    );
                }

                let out = line;

                // ORDER IS CRITICAL — most-specific patterns FIRST
                // Each uses a check so it won't match already-processed or complex tokens

                // 1. stadium  ([label])
                out = out.replace(/\b(\w[\w-]*)\s*\(\[([^\]]*)\]\)/g,
                    (_m, id, lbl) => isQ(lbl) || lbl.includes('(') || lbl.includes('[') || lbl.includes('"') ? _m : `${id}([${q(lbl)}])`);

                // 2. circle  ((label))
                out = out.replace(/\b(\w[\w-]*)\s*\(\(([^)]*)\)\)/g,
                    (_m, id, lbl) => isQ(lbl) || lbl.includes('(') || lbl.includes('[') || lbl.includes('{') || lbl.includes('"') ? _m : `${id}((${q(lbl)}))`);
                ...
                // 4. subroutine  [[label]]
                out = out.replace(/\b(\w[\w-]*)\s*\[\[([^\]]*)\]\]/g,
                    (_m, id, lbl) => isQ(lbl) || lbl.includes('[') || lbl.includes('"') ? _m : `${id}[[${q(lbl)}]]`);
                ...
                // 9. edge labels  -->|label|
                out = out.replace(/(-->|--[->]|===>|-.->)\s*\|([^"|]+)\|/g,
                    (_m, arr, lbl) => isQ(lbl) || lbl.includes('"') ? _m : `${arr}|${q(lbl)}|`);
